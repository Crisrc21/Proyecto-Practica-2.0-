...
import { accumulateAssembly, activeStages, advancePayment, isAdvanceMilestone, latestReport, metricLabels, metricValue, moveWorkflowMilestone, orderedWorkflowMilestones, natures, physicalPercent, stages, workflowAtCutoff } from "../data/project-workflow";
...
export function PotentialContracts({ projects, busy, onEdit, onSign, onAdd, onDelete }: { projects: Project[]; busy: boolean; onEdit: (p: Project) => void; onSign: (p: Project) => void; onDelete: (p: Project) => void; onAdd: () => void }) {
  return <section className="project-panel"><div className="project-panel-heading"><div><span className="project-eyebrow">PASO 0</span><h2>Contratos potenciales</h2><p>Agrega un nombre. Completa la ficha cuando el contrato esté firmado.</p></div><button className="project-button primary" onClick={onAdd}><Plus size={16} />Agregar potencial</button></div>
    ...
    {projects.map(p => <article className="project-potential-row" key={p.id}><div><span className="project-badge neutral">Pendiente de firma</span><h3><button className="project-text-link" onClick={() => onEdit(p)}>{p.name}</button></h3></div><div className="project-potential-actions"><button className="project-button" onClick={() => onEdit(p)}>Editar nombre</button><button className="project-button primary" disabled={busy} onClick={() => onSign(p)}><CheckCircle2 size={16} />Contrato firmado</button><button className="project-button" disabled={busy} onClick={() => onDelete(p)} aria-label={`Eliminar proyecto ${p.name}`}><Trash2 size={15} />Eliminar proyecto</button><small>Fecha de firma: {p.signedDate ? shortDate(p.signedDate) : "hoy"}</small></div></article>)}
  </section>;
}

export function ProjectWorkflow({ project, invoices, dataState, cutoff, onSave, onEditStage, onEditProject, onEdps, onCash, onCorrect }: { project: Project; invoices: FacturaCalculada[]; dataState: DataState; cutoff: string; onSave: Save; onEditStage: (m: Milestone) => void; onEditProject: () => void; onEdps: () => void; onCash: () => void; onCorrect: (r: ProgressReport) => Promise<void> }) {
  ...
  const ordered = orderedWorkflowMilestones(project);
  async function reorder(source: string, target: string, after: boolean) {
    if (busy) return;
    const ids = ordered.map(m => m.id);
    const next = moveWorkflowMilestone(ids, source, target, after);
    setDragId(null); setDropAt(null);
    if (next.every((id, index) => id === ids[index])) return;
    setBusy(true); setOrderMessage("");
    try { await onSave({ ...project, milestoneOrder: next }, "Orden del recorrido actualizado");  }
    catch (err) { setOrderMessage(err instanceof Error ? err.message : "No se pudo guardar el orden. Intenta nuevamente."); }
  ...
  return <div className="project-stack"><section className="project-panel project-workflow-intro"><div className="project-section-intro"><div><span className="project-eyebrow">PASO 2 · SEGUIMIENTO</span><h2>Hitos y avances del contrato</h2><p>Naturaleza {project.nature} · {natures.find(n => n.id === project.nature)?.label}</p></div><span className="project-badge neutral"><Clock3 size={13} />Al {shortDate(cutoff)}</span></div></section>
    <div className="project-workspace"><aside className="project-panel project-stage-navigation"><div className="project-panel-heading"><div><h3>Recorrido del proyecto</h3><p>Selecciona una etapa para ver su evolución.</p></div></div><button className="project-button project-add-milestone" onClick={() => { setError(""); setAdding(true); }}><Plus size={15} />Agregar hito</button>{orderMessage && <p role="alert" className="project-error project-reorder-help">{orderMessage}</p>}<ol className="project-stage-rail" aria-label="Orden de los hitos" aria-busy={busy}>{ordered.map((m, index) => {
      const value = m.stageKey ? progress(m.stageKey) : m.actualDate && m.actualDate <= cutoff ? 100 : null;
      const selected = m.stageKey ? !custom && stage.key === m.stageKey : custom?.id === m.id;
      return <li key={m.id} draggable={!busy} className={`${value === 100 ? "is-complete" : ""} ${selected ? "is-selected" : ""} ${dragId === m.id ? "is-dragging" : ""} ${dropAt?.id === m.id ? dropAt.after ? "drop-after" : "drop-before" : ""}`}
        onDragStart={e => { if (busy) { e.preventDefault(); return; } setDragId(m.id); e.dataTransfer.effectAllowed = "move"; e.dataTransfer.setData("text/plain", m.id); }}
        onDragOver={e => { if (!dragId || busy || dragId === m.id) return; e.preventDefault(); e.dataTransfer.dropEffect = "move"; const rect = e.currentTarget.getBoundingClientRect(); const horizontal = window.matchMedia("(max-width:760px)").matches; setDropAt({ id: m.id, after: horizontal ? e.clientX > rect.left + rect.width / 2 : e.clientY > rect.top + rect.height / 2 }); }}
        onDrop={e => { e.preventDefault(); if (dragId && dropAt?.id === m.id) void reorder(dragId, m.id, dropAt.after); }}
        onDragEnd={() => { setDragId(null); setDropAt(null); }}>
        <button aria-pressed={selected} aria-label={`${m.name}, posición ${index + 1} de ${ordered.length}. Alt más flechas para mover.`} disabled={busy} onKeyDown={e => { if (!e.altKey || !["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight"].includes(e.key)) return; e.preventDefault(); const down = e.key === "ArrowDown" || e.key === "ArrowRight"; const target = ordered[index + (down ? 1 : -1)]; if (target) void reorder(m.id, target.id, down); }} onClick={() => { if (m.stageKey) { setSelectedKey(m.stageKey); setCustomId(null); } else setCustomId(m.id); }}><span className="project-rail-dot">{value === 100 ? <Check size={15} /> : index + 1}</span><span className="project-rail-label"><strong>{m.name}</strong><small>{m.stageKey === "anticipo" ? advance.status : value === 100 ? "Cumplido" : m.stageKey ? value !== null ? "Avance registrado" : "Sin medición" : m.state}</small></span>{m.stageKey && <span className="project-rail-value">{number(value)}{value !== null ? "%" : ""}</span>}</button>
      </li>;
    })}</ol></aside>
    <div className="project-stage-detail project-stack">{custom ? <section className="project-panel"><div className="project-panel-heading"><div><span className="project-eyebrow">HITO DEL CONTRATO</span><h2>{custom.name}</h2><p>{custom.scope} · {custom.actualDate && custom.actualDate <= cutoff ? "Cumplido" : custom.state}</p></div><div className="project-actions"><button className="project-button" onClick={() => onEditStage(custom)}>Actualizar hito</button><button className="project-button" onClick={() => { setError(""); setRemoving(custom); }}><Trash2 size={15} />Eliminar hito</button></div></div><div className="project-date-strip"><div><span>Compromiso inicial</span><strong>{shortDate(custom.baselineDate)}</strong></div><div><span>Fecha prevista actual</span><strong>{shortDate(custom.forecastDate)}</strong></div><div><span>Cumplimiento efectivo</span><strong>{shortDate(custom.actualDate && custom.actualDate <= cutoff ? custom.actualDate : null)}</strong></div></div>{custom.notes && <p>{custom.notes}</p>}<button className="project-button" onClick={onCash}>Consultar cobros</button></section> : <><section className="project-panel"><div className="project-panel-heading"><div><span className="project-eyebrow">HITO {ordered.findIndex(m => m.stageKey === stage.key) + 1} DE {ordered.length}</span><h2>{stage.label}</h2></div><div className="project-actions">{!["firma", "anticipo"].includes(stage.key) && <button className="project-button primary" onClick={() => setEditing(stage.key)}><Plus size={15} />Registrar avance</button>}{stage.key === "firma" && <button className="project-button" onClick={onEditProject}>Ver ficha contractual</button>}{stage.key === "anticipo" && <button className="project-button primary" onClick={onCash}>Consultar cobros</button>}{rawMilestone && stage.key !== "firma" && <button className="project-button" onClick={() => onEditStage(rawMilestone)}>Programar hito</button>}{rawMilestone && <button className="project-button" onClick={() => { setError(""); setRemoving(rawMilestone); }}><Trash2 size={15} />Eliminar hito</button>}</div></div>
    ...
    {stage.key === "anticipo" ? <section className="project-panel"><div className="project-panel-heading"><div><h3>Historia de cobros del anticipo</h3><p>Pagos y abonos efectivamente registrados en CxC.</p></div></div><HistoryLine key={`cash-${project.id}`} events={cashEvents} empty={advance.label} /></section> : <ProgressHistory onEdit={setEditingReport} key={stage.key} project={project} stage={stage.key} cutoff={cutoff} />}
    </>}</div></div>
    ...
    {editingReport && <ProgressForm key={editingReport.id} initial={editingReport} onCorrect={onCorrect} project={project} stage={editingReport.stageKey} onSave={onSave} onClose={() => setEditingReport(null)} />}
...
function ProgressForm({ project, stage, onSave, onClose, initial, onCorrect }: { project: Project; stage: StageKey; onSave: Save; onClose: () => void; initial?: ProgressReport; onCorrect?: (r: ProgressReport) => Promise<void> }) {
  const previous = latestReport(project, stage, todayISO());
  const chronology = (project.progressReports || []).filter(r => r.stageKey === stage).sort((a,b) => a.date.localeCompare(b.date));
  const beforeInitial = initial ? chronology[chronology.findIndex(r => r.id === initial.id) - 1]?.quantity ?? 0 : 0;
  const [draft, setDraft] = useState<ProgressReport>(initial ? { ...initial, quantity: stage === "montaje" ? (initial.quantity ?? 0) - beforeInitial : initial.quantity } : { id: createProjectRecordId(), stageKey: stage, date: todayISO(), evidence: "", quantity: stage === "montaje" ? null : previous?.quantity ?? null, completedUnits: previous?.completedUnits ?? null, percent: previous?.percent ?? null });
  const reason = "Actualización de avance acumulado";
  const [busy, setBusy] = useState(false), [error, setError] = useState("");
  async function submit(e: FormEvent) {
    e.preventDefault(); setBusy(true); setError("");
    try { if (initial && onCorrect) { await onCorrect(draft); onClose(); return; } const report = stage === "montaje" ? accumulateAssembly(project, draft) : draft; await onSave({ ...project, progressReports: [...(project.progressReports || []), report] }, stage === "montaje" ? `Registro de ${draft.quantity} casas montadas; acumulado ${report.quantity}` : reason); onClose(); }
    catch (err) { setError(err instanceof Error ? err.message : "No se pudo guardar el avance."); } finally { setBusy(false); }
  }
  const quantityLabel = stage === "fabricacion" ? "m² fabricados acumulados" : stage === "despachos" ? "Casas despachadas acumuladas" : stage === "montaje" ? "Casas montadas en este registro" : "Casas con fundación terminada";
  return <Editor title={`${initial ? "Editar avance" : "Registrar avance"} · ${stages.find(s => s.key === stage)?.label}`} onClose={onClose}><form className="project-form" onSubmit={submit}>
    <p className="project-note project-full">{initial ? "Corrige la fecha, la cantidad o la evidencia. En Montaje ingresa las casas de este registro: se recalcularán los acumulados posteriores. La corrección quedará en el historial." : stage === "montaje" ? "Ingresa solo las casas nuevas montadas. Se sumarán automáticamente al acumulado al guardar y se revisarán los EDP de esta etapa." : "Registra el total acumulado, no el incremento del período. Se revisarán las condiciones de los EDP de esta etapa al guardar."}</p>
    <label className="project-field">Fecha del corte<input required type="date" min={stage === "montaje" && !initial ? previous?.date || project.signedDate || undefined : project.signedDate || undefined} max={todayISO()} value={draft.date} onChange={e => setDraft(d => ({ ...d, date: e.target.value }))} /></label>
    {["fabricacion", "despachos", "fundaciones", "montaje"].includes(stage) && <label className="project-field">{quantityLabel}<input type="number" required={stage === "montaje"} min={stage === "montaje" && !initial ? 1 : 0} step={stage === "fabricacion" ? "any" : "1"} max={stage === "fabricacion" ? project.areaM2 ?? undefined : stage === "montaje" && !initial ? Math.max(0, project.units - (previous?.quantity ?? 0)) : project.units} value={draft.quantity ?? ""} onChange={e => setDraft(d => ({ ...d, quantity: e.target.value === "" ? null : Number(e.target.value) }))} /></label>}
    {stage === "montaje" && !initial && <p className="project-note project-full">Acumulado actual: {previous?.quantity ?? 0} casas. Al guardar: {(previous?.quantity ?? 0) + (draft.quantity ?? 0)} de {project.units} casas.</p>}
...
export function ProjectEdps({ project, cutoff, invoices, dataState, onEdit, onCash, onDelete }: { project: Project; cutoff: string; invoices: FacturaCalculada[]; dataState: DataState; onEdit: (m: Milestone) => void; onCash: () => void; onDelete: (m: Milestone) => Promise<void> }) {
  ...
  async function removeEdp() {
    if (!deleting || deleteBusy) return;
    setDeleteBusy(true); setDeleteError("");
    try { await onDelete(deleting); setDeleting(null); setSelectedId(null); }
    catch (err) { setDeleteError(err instanceof Error ? err.message : "No se pudo eliminar el EDP."); }
  ...
  return <div className="project-stack"><section className="project-panel project-workflow-intro"><div className="project-section-intro"><div><span className="project-eyebrow">PASO 3 · ESTADOS DE PAGO</span><h2>Del avance al cobro</h2><p>Selecciona un EDP para revisar su recorrido y el siguiente paso.</p></div><button className="project-button primary" disabled={!project.nature} onClick={add}><Plus size={16} />Planificar EDP</button></div>{!project.nature && <p>Selecciona la naturaleza en la ficha para habilitar nuevos EDP por avance.</p>}</section>
    <>{deleting && <Editor title="Eliminar EDP" onClose={() => { if (!deleteBusy) setDeleting(null); }}><div className="project-form"><div className="project-full"><p>¿Eliminar <strong>«{deleting.name}»</strong> del contrato <strong>{project.name}</strong>?</p><p>Se quitará de los estados de pago y de las proyecciones. Sus {project.invoiceLinks.filter(l => l.milestoneId === deleting.id).length} facturas vinculadas quedarán disponibles para otra asociación. Las facturas y pagos de CxC se conservarán.</p><p>La eliminación quedará registrada en el historial.</p></div>{deleteError && <p role="alert" className="project-error project-full">{deleteError}</p>}<div className="project-form-actions project-full"><button className="project-button" disabled={deleteBusy} onClick={() => setDeleting(null)}>Cancelar</button><button className="project-button primary" disabled={deleteBusy} onClick={removeEdp}>{deleteBusy ? "Eliminando…" : "Eliminar EDP"}</button></div></div></Editor>}</><div className="project-edp-filters" role="group" aria-label="Filtrar estados de pago">{["Todos", "Previsto", "Preparado", "Presentado", "Aprobado", ...(edps.some(m => !m.edp) ? ["Sin condición"] : [])].map(state => <button key={state} aria-pressed={filter === state} className={filter === state ? "is-active" : ""} onClick={() => setFilter(state)}><span>{state === "Todos" ? "Todos los EDP" : state === "Previsto" ? "Por habilitar" : state === "Preparado" ? "Preparados" : state === "Presentado" ? "Presentados" : state === "Aprobado" ? "Aprobados" : state}</span><strong>{edps.filter(m => state === "Todos" || (m.edp?.status || "Sin condición") === state).length}</strong></button>)}</div>
...
function EdpDetail({ project, milestone: m, cutoff, invoices, dataState, onEdit, onCash, attach, onDelete }: { project: Project; milestone: Milestone; cutoff: string; invoices: FacturaCalculada[]; dataState: DataState; onEdit: () => void; onCash: () => void; attach: boolean; onDelete: () => void }) {
  ...
  return <div className="project-stack project-edp-detail"><section className="project-panel"><div className="project-panel-heading"><div><span className="project-eyebrow">{edp ? stages.find(s => s.key === edp.stageKey)?.label : m.scope}</span><h2>{m.name}</h2></div><div className="project-actions"><button className="project-button" onClick={onDelete}><Trash2 size={15} />Eliminar EDP</button><button className="project-button primary" onClick={onEdit}>{attach ? "Definir condición" : "Gestionar EDP"}</button></div></div>
...
export function EdpForm({ project, milestone, onSave, onClose }: { project: Project; milestone: Milestone; onSave: (m: Milestone, reason: string) => Promise<void>; onClose: () => void }) {
  ...
  return <Editor title={milestone.name ? "Gestionar estado de pago" : "Planificar estado de pago"} onClose={onClose}><form className="project-form" onSubmit={submit}>
    ...
    <label className="project-field">Umbral acumulado<input type="number" required disabled={locked || edp.metric === "firma"} min={edp.metric.endsWith("Pct") || edp.metric === "m2" ? 0.001 : 1} max={edp.metric.endsWith("Pct") ? 100 : edp.metric === "m2" ? project.areaM2 ?? undefined : edp.metric === "firma" ? 1 : project.units} step={edp.metric.endsWith("Pct") || edp.metric === "m2" ? "any" : "1"} value={edp.threshold} onChange={e => condition({ threshold: Number(e.target.value) })} /></label>
