...
import { ArrowLeft, ArrowUpRight, BookOpen, Building2, CalendarDays, CalendarRange, CheckCircle2, CircleAlert, ClipboardList, ExternalLink, History, Plus, RefreshCw, Trash2, Wallet } from "lucide-react";
...
import { deleteProject, deleteProjectEdp, editProjectProgress, loadProjects, saveProject } from "../data/projects-api";
...
import { Editor, MilestoneForm, PotentialForm, ProjectForm } from "./project-forms";
...
export function ProjectsView({ search, invoices, dataState, onRefreshAccounts }: { search: string; invoices: FacturaCalculada[]; dataState: DataState; onRefreshAccounts: () => Promise<void> }) {
  ...
  function confirmDelete(project: Project) { setDeleteError(""); setDeleteTarget(project); }
  async function removeProject() {
    ...
    try {
      ...
      if (selectedId === deleteTarget.id) changeView("resumen", null);
      setMessage(`Proyecto «${deleteTarget.name}» eliminado.`);
    } catch (err) { setDeleteError(err instanceof Error ? err.message : "No se pudo eliminar el proyecto."); }
  ...
  return <div className="projects-module">
    ...
    {view === "potenciales" && <PotentialContracts projects={selected ? [selected] : potentials} busy={busy} onEdit={setProjectEditor} onSign={signContract} onDelete={confirmDelete} onAdd={() => setProjectEditor(emptyProject())} />}
    {view === "avances" && selected && <ProjectWorkflow onCorrect={async report => { const saved = await editProjectProgress(selected, report); setProjects(current => current.map(p => p.id === saved.id ? saved : p)); setMessage("Avance corregido. Acumulados y EDP actualizados."); }} key={selected.id} project={selected} invoices={invoices} dataState={dataState} cutoff={cutoff} onSave={persist} onEditStage={setMilestoneEditor} onEditProject={() => setProjectEditor(selected)} onEdps={() => changeView("edps")} onCash={() => changeView("cobros")} />}
    {view === "edps" && selected && <ProjectEdps onDelete={async m => { const saved = await deleteProjectEdp(selected, m.id); setProjects(current => current.map(p => p.id === saved.id ? saved : p)); setMessage(`EDP «${m.name}» eliminado.`); }} key={selected.id} project={selected} cutoff={cutoff} invoices={invoices} dataState={dataState} onEdit={setMilestoneEditor} onCash={() => changeView("cobros")} />}
    ...
    {deleteTarget && <Editor title="Eliminar proyecto" onClose={() => { if (!busy) setDeleteTarget(null); }}><div className="project-form"><div className="project-full"><p>¿Eliminar el proyecto <strong>«{deleteTarget.name}»</strong>?</p><p>Se quitará de la cartera, los indicadores, las líneas de tiempo y las proyecciones. Sus {deleteTarget.milestones.length} hitos dejarán de aparecer.</p><p>Las facturas y pagos de CxC se conservarán. Las {deleteTarget.invoiceLinks.length} facturas vinculadas quedarán disponibles para asociar a otro proyecto.</p></div>{deleteError && <p className="project-error project-full" role="alert">{deleteError}</p>}<div className="project-form-actions project-full"><button className="project-button" disabled={busy} onClick={() => setDeleteTarget(null)}>Cancelar</button><button className="project-button primary" disabled={busy} onClick={removeProject}>{busy ? "Eliminando…" : "Eliminar proyecto"}</button></div></div></Editor>}
